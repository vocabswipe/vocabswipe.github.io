            ...
            for i, line in enumerate(lines, 1):
                ...
                try:
                    ...
                    if line.count('{') > line.count('}'):
                        line += '}'
                    # Fix 3: Add missing opening brace
                    if line.count('}') > line.count('{'):
                        line = '{' + line
                    # Fix 4: Remove trailing comma in lists/objects
                    line = line.replace(',]', ']').replace(',}', '}')
                    # Fix 5: Ensure proper quotes
                    line = line.replace("'", '"')
...
def check_duplicates(vocab_db):
    """Check and remove duplicate entries in the database."""
    seen_words = set()
    duplicates = []
