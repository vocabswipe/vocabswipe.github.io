...
def is_non_english_word(word):
    """Check if a word contains non-English characters (e.g., Chinese, Russian, Thai)."""
    if not word:
        return False
    # Define Unicode ranges for common non-English scripts
    non_english_ranges = [
    ...
    for char in word:
        ...
        for start, end in non_english_ranges:
            if start <= codepoint <= end:
                ...
                logger.warning(f"Non-English character detected in word '{word}': {char} ({script})")
                return True
    return False
        ...
        if file_type == 'jsonl':
            ...
            for i, line in enumerate(lines, 1):
                ...
                try:
                    ...
                    if is_non_english_word(entry.get('word', '')):
                        non_english_words.append((i, entry.get('word', ''), line))
                        continue
                    ...
                    try:
                        ...
                        if is_non_english_word(entry.get('word', '')):
                            non_english_words.append((i, entry.get('word', ''), fixed_line))
                            continue
                        entries.append(entry)
            ...
            if non_english_words:
                ...
                for line_num, word, content in non_english_words:
                    table.add_row(str(line_num), word, content[:50] + ('...' if len(content) > 50 else ''))
                console.print(table)
                console.print(Panel(
                    f"[yellow]⚠ {len(non_english_words)} non-English words skipped in processing.[/yellow]",
...
def check_non_english_in_db(vocab_db):
    """Check for non-English words in the database and report them."""
    non_english_words = []
    for entry in vocab_db:
        if is_non_english_word(entry['word']):
            non_english_words.append(entry['word'])
    return non_english_words

def generate_summary_report(vocab_db, valid_entries, duplicates, removed_count, duplicate_sentences, removed_sentence_count, missing_audio, non_english_words, first_word, last_word):
    """Generate a concise summary report with non-English words and confirmation."""
    ...
    table.add_row("Non-English Words in Input", f"[yellow]{len([e for e in valid_entries if is_non_english_word(e['word'])])}[/yellow]")
    table.add_row("Non-English Words in DB", f"[yellow]{len(non_english_words)}[/yellow]" if non_english_words else "[green]0[/green]")
    ...
    if non_english_words:
        console.print(Panel(
            "\n".join([f"Word: {word}" for word in non_english_words]),
            title="Non-English Words in Database", border_style="yellow", expand=False
        ))
        console.print(Panel(
            "[yellow]⚠ Non-English words found in database. Please manually remove them from vocab3000_database.yaml.[/yellow]",
    ...
    confirmation.append("[green]✓ Database Within 3000-Word Limit[/green]" if total_words <= DATABASE_WORD_LIMIT else "[red]✗ Database Exceeds 3000-Word Limit[/red]")
...
def process_entries(entries):
    """Process entries with a single-line progress bar."""
    ...
    with Progress(
        ...
        for entry in entries:
            ...
            word_lower = entry['word'].lower()
            # Skip if word not in database (enforce 3000-word limit)
            if not any(e['word'].lower() == word_lower for e in vocab_db):
                skipped_words.append(entry['word'])
                logger.warning(f"Word '{entry['word']}' not in database. Skipped due to 3000-word limit.")
                progress.advance(task)
                continue
            ...
            # If word exists in DB but no entry with Matthew voice, skip (only update existing entries)
            logger.warning(f"Word '{entry['word']}' found in DB but no Matthew voice entry. Skipped.")
            skipped_words.append(entry['word'])
            progress.advance(task)
            continue

    if skipped_words:
        table = Table(title="⚠ Skipped Words (Not in Database)", style="yellow", show_lines=True)
        table.add_column("Word", style="bold")
        for word in skipped_words:
            table.add_row(word)
        console.print(table)
        console.print(Panel(
            f"[yellow]⚠ {len(skipped_words)} words skipped as they are not in the 3000-word database.[/yellow]",
            title="Skipped Words", border_style="yellow", expand=False
        ))
    ...
    non_english_words = check_non_english_in_db(vocab_db)
    save_file(vocab_db, VOCAB_DB_PATH, file_type='yaml')
    generate_summary_report(vocab_db, valid_entries, duplicates, removed_count, duplicate_sentences, removed_sentence_count, missing_audio, non_english_words, first_word, last_word)
...
def main():
    ...
    if vocab_db:
        ...
        duplicate_sentences, removed_sentence_count = check_duplicate_sentences(vocab_db)
        non_english_words = check_non_english_in_db(vocab_db)
        if removed_count > 0 or removed_sentence_count > 0 or non_english_words:
            save_file(vocab_db, VOCAB_DB_PATH, file_type='yaml')
            logger.info(f"Initial check: Removed {removed_count} word entries, {removed_sentence_count} sentences, found {len(non_english_words)} non-English words.")
