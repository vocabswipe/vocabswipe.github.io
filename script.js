document.addEventListener('DOMContentLoaded', () => {
  ...
  let wordToSentenceMap = new Map();
  let wordSizeCache = new Map(); // Cache for word dimensions
  ...
  function getWordSize(word, fontSize) {
    if (wordSizeCache.has(`${word}_${fontSize}`)) {
      return wordSizeCache.get(`${word}_${fontSize}`);
    }
    const tempEl = document.createElement('div');
    tempEl.style.position = 'absolute';
    tempEl.style.visibility = 'hidden';
    tempEl.style.fontSize = `${fontSize}rem`;
    tempEl.style.fontFamily = "'Roboto', sans-serif";
    tempEl.style.fontWeight = '700';
    tempEl.textContent = word;
    document.body.appendChild(tempEl);
    const { width, height } = tempEl.getBoundingClientRect();
    document.body.removeChild(tempEl);
    wordSizeCache.set(`${word}_${fontSize}`, { width, height });
    return { width, height };
  }
      ...
      if (cachedData) {
        ...
        if (entries.length > 0) {
          ...
          setTimeout(() => {
            ...
            // Compute word-to-sentence map asynchronously
            computeWordToSentenceMap(() => displayWordCloud(0));
          }, 300);
          return;
        }
      }
  ...
  function computeWordToSentenceMap(callback) {
  ...
  function displayWordCloud(startIndex = 0) {
    ...
    const initialDisplayCount = Math.ceil(wordArray.length * 0.01); // Reduced to 1% for faster load
    const maxAttempts = 50; // Further reduced attempts
    const maxWords = 100; // Limit total words to prevent overload

    function placeWord({ word, freq }, index, callback) {
      ...
      const { width, height } = getWordSize(word, size);
    ...
    // Place initial words synchronously
    wordArray.slice(0, initialDisplayCount).forEach((wordObj, index) => {
      placeWord(wordObj, index);
    });

    // Place remaining words asynchronously with small delay
    const remainingWords = wordArray.slice(initialDisplayCount, maxWords);
    let wordIndex = 0;

    function placeNextWord() {
      if (wordIndex >= remainingWords.length) {
        // Draw connecting lines after all words are placed
        requestAnimationFrame(() => {
          ...
          placedWords.forEach((word1, i) => {
            const nearest = placedWords
              ...
              .slice(0, 4); // Reduced to 4 connections for performance
      ...
      placeWord(remainingWords[wordIndex], initialDisplayCount + wordIndex, () => {
        wordIndex++;
        setTimeout(placeNextWord, 10); // Small delay to yield to browser
      });
    }

    if (remainingWords.length > 0) {
      setTimeout(placeNextWord, 10);
    } else {
      placeNextWord();
    }
  }
